...
legalOS is built to serve one corporate legal department at a time (single-tenant), with a multi-tenant-ready schema so the same codebase can later support a SaaS version for multiple legal departments. It ships with 11 departments organized into four product clusters — deal & transactional (Commercial, Corporate), regulatory & compliance (Regulatory, Privacy, AI Governance), specialized practice (Product, Employment, IP, Litigation), and operational & utility (Operations, General Tools) — and is designed so adding more is mostly configuration. (Public Sector and Compliance were soft-deleted in migration 0043, so the committed default ship is 11, not the historical 13.)
