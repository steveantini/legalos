 ...
 *
 * `superAdminCount` lets each row decide whether its user is the org's only
 * super_admin (the last-super-admin lockout guard) without every row recomputing
 * it. `actorRole` / `actorUserId` thread the viewing admin's authority down so
 * each row renders the role editor honestly.
 */
        ...
        <ul className="mt-4 flex flex-col gap-2">
          {users.map((u) => (
            <PersonRow
              ...
              isOnlySuperAdmin={u.role === "super_admin" && superAdminCount === 1}
            />
          ))}
        </ul>
      ) : (
