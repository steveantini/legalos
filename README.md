...
- **Document export.** Export agent conversations and individual responses to Word (.docx), Google Docs, and additional document formats. Per-message Word (.docx) export ships today via the kebab destination hub on the chat message action row (with citation footnotes, a Sources section, and a branded footer); full-conversation export and additional formats (Google Docs, and similar) are deferred to the Share & connector hub (roadmap item 2).
