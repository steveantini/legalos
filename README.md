...
**Phase 2 polish phase.** Three-tier agent architecture (canonical Department Agents, Claude for Legal imports, user-owned My Agents) across a 13-department launchpad behind RBAC. Native chat with prompt caching, web search, attached references, per-message file attachments (uploads from the local desktop; connected drives deferred to the Share & connector hub), per-message markdown download, soft delete + 30-day undo, and an admin area are live. See [`PROJECT_OUTLINE.md`](./PROJECT_OUTLINE.md) for the full phase plan.
