...
- **2b — the MCP client + OAuth 2.1 connect/auth + tools/list at connect (next).** Add the MCP client (the @modelcontextprotocol/sdk is the standard dependency), generalize the connect/callback flow for the MCP adapter's discovery-backed OAuth 2.1 (reusing the PKCE/TokenBundle substrate), and fetch the server's tools/list at connect time; enforce isTrustedFirstPartyServer at both initiate and callback plus the partitioned self-hosted path.
