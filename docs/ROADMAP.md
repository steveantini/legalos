...
- **Next: 2P-3 — single-tool execution** (`getUsableAccessToken` → `callMcpServerTool` → shape a `tool_result` (+ typed errors), reusing the Phase 1 client + token layer). Then 2P-4 (read/write classification) → 2P-5 (per-agent governance) → 2P-6 (the gated loop, the one hot-path change) → 2P-7 (UI + optional interactive write-confirm).
