...
**MODELS-AS-A-CONNECTION (flag 1: 1a-1d) is COMPLETE.** An organization can bring its own Anthropic key or use the managed default, configured in-product. BYO Anthropic is now usable end to end (the go-live proof-out's first connection). Next in the Connections phase build, per the decided architecture: MCP (flag 2) as a first-class connection kind, and/or finishing the OAuth data-source connectors (Mail, Calendar) toward the go-live proof-out (connect real services on the operator's instance: BYO Anthropic now usable; Drive / Mail / Calendar via MCP if Google offers official first-party servers, else OAuth). Then models-as-a-connection leads into the Business model & pricing arc (above), which unblocks A4b.
