...
- **2b — the MCP client + OAuth 2.1 connect/auth + tools/list at connect.** Decomposed into three steps (the chat hot path stays untouched throughout Phase 1):
  ...
  - **2b-ii — the OAuth 2.1 connect/auth flow + trust-gated connection (next).** Generalize the connect/callback flow for the MCP adapter's discovery-backed OAuth 2.1 (reusing the PKCE/TokenBundle substrate), enforce isTrustedFirstPartyServer at both initiate and callback plus the partitioned self-hosted path.
