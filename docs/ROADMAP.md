...
- **Next: 2P-5 — per-agent governance** (the agent author enables which connected servers an agent may use; agent-form UI + the resolver that filters the 2P-2 tool list). Then 2P-6 (the gated loop, the one hot-path change) → 2P-7 (UI + optional interactive write-confirm).
