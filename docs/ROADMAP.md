...
- **1d — the connector UI (next).** The "best agnostic model connector" surface in Policy & access over the 1c actions: managed-vs-BYO configuration, key entry with validation feedback, the masked hint, a provider grid (Anthropic available; Google / OpenAI / self-hosted coming soon), two-axes-legible (which provider / whose key) and provider-uniform, with models flowing in after the connection is configured. Built against the frontend-design skill.
