...
- **Next: 2P-2 — tool mapping + namespacing** (connected tools → namespaced Anthropic custom tool defs + the routing map; widen the `AnthropicTool` union). Then 2P-3 (single-tool execution) → 2P-4 (read/write classification) → 2P-5 (per-agent governance) → 2P-6 (the gated loop, the one hot-path change) → 2P-7 (UI + optional interactive write-confirm).
