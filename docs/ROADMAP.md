...
- **2b — the MCP client + OAuth 2.1 connect/auth + tools/list at connect.** Decomposed into three steps (the chat hot path stays untouched throughout Phase 1):
  ...
  - **2b-ii — the OAuth 2.1 connect/auth flow + trust-gated connection.** Approach: Option A (use the @modelcontextprotocol/sdk's discrete step-functions — discovery, registration, exchange, refresh — which return secrets to us, and orchestrate via our own routes/crypto/tokens so custody stays in our encrypted substrate; the control-plane principle). Decomposed into three steps (the chat hot path stays untouched):
    ...
    - **2b-ii-2 — the MCP adapter + connect/callback MCP branch (next).** The McpServerAdapter implementing discovery-backed buildAuthorizationUrl/exchangeCode (via the SDK's startAuthorization/exchangeAuthorization over discovered metadata + stored/registered client info), the trust gate at initiate + callback, proven against a self-hosted MCP test server first; MCP refresh fills in its branch here.
    - **2b-ii-3 — the self-hosted partitioned path.** The super-admin customer-URL flow, trust tier 'self_hosted' derived from the path, the self-hosted flag carried in signed state, kept separate from the first-party flow.
