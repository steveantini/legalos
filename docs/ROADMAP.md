...
**A4a (Insights usage/adoption lens) shipped (2026-06-01):** the first MEASURE area's first lens, a measured view of native-agent activity built on `usage_events` by generalizing the home impact-math query pattern org-wide. Total agent runs and trend over a chosen window (week/month/YTD), broken down by agent, department (via the agent, the clean rollup), model, and person, plus an adoption-gap signal (agents with zero runs). A clearly-labeled demo sample-data toggle previews the experience before real usage accrues. Org-admin gated; no cost shown; JS-side aggregation at current volume with a noted SQL-side path. See D-082.
...
Remaining admin areas: **A5 Evals** (MEASURE side); then **A7** docs close-out. Deferred MEASURE delight passes: **A4b** (Insights cost/ROI, gated on the business-model arc) and Evals depth. Deferred ops/polish, not blocking: a branded custom invite-email sender (Resend + `generateLink`) and a Supabase SMTP configuration for invite volume. The remaining legacy admin pages (Productivity Calculator, Adoption Metrics) stay reachable at their routes but unlinked from the new rail, retiring once Insights fully covers their intent (after A4b).
