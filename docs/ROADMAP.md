...
- **Next: 2P-4 — read/write annotation capture + classifier** (capture `readOnlyHint`/`destructiveHint` at discovery; classify, no-annotation ⇒ write). Then 2P-5 (per-agent governance) → 2P-6 (the gated loop, the one hot-path change) → 2P-7 (UI + optional interactive write-confirm).
