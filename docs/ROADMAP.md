...
- **Next: 2P-4b — refresh an already-connected server's tools** to backfill annotations into stored catalogs (the Google servers were discovered before 2P-4, so their stored catalogs are annotation-less and classify as write until refreshed). Then 2P-5 (per-agent governance) → 2P-6 (the gated loop, the one hot-path change) → 2P-7 (UI + optional interactive write-confirm).
