...
Remaining People milestones: **A3b** soft-deactivation (makes the inert `users.is_active` load-bearing), **A3c** invitation (net-new; replaces the env-var allowlist), after which the old users page is retired. Then the other areas: **A4 Insights** (absorbing today's Adoption Metrics and Productivity Calculator), **A5 Evals**, **A6** the audit-log viewer (over `role_change_audit` and later audit streams); data/retention folds into People/Policy; then a docs close-out. The existing admin pages (Productivity Calculator, Adoption Metrics, User Access) stay reachable at their routes but unlinked from the new rail, retiring per-area as their replacements ship.
