...
**Per-row dominance tuning (2026-06-30).** Every row is asymmetric, but the dominant element alternates by section, set per-row on `FeatureRow` via two knobs (`proseDominant` plus the implied column split and window scale). Workspace, Agents, and Knowledge are PROSE-dominant: the prose takes the wide `1fr` track and the window sits in an `auto` track, laid out at a comfortable 600px internal width and zoomed to 0.82 (about 490px visual) so it reads as a supporting visual yet stays legible (the surfaces need ~600px internal for the fixed rail, so the box is scaled, not the layout). Workflows and Governance stay WINDOW-dominant (a 1.55fr window track, about 660px, no zoom). The left/right alternation is unchanged.
