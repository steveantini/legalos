...
**Shipped:** Sessions 8a–25. Recent work: routing migration to `/workspace` prefix (D-036), `unpdf` swap for serverless-safe PDF extraction, marketing landing at `/` with two-phase glyph choreography, palette retune proportional to sRGB headroom (D-037), Vercel Analytics + Speed Insights wired, login surface state machine + visual polish + authed-user bounce (D-039), custom SMTP via Resend in sandbox mode lifts the 2/hour magic-link rate limit with delivery scoped to operator email pending D-036 (D-040), `?next=` proxy preservation completes D-036's deferral + retires three Phase-0/1 config TODOs + fixes a latent session-cookie-carry bug in the unauthed proxy redirect; CHANGELOG backfilled for Sessions 20–21 (RLS migration 0015 + empty-state polish; rail restructure + Commercial agents flipped to native + adaptive welcome hero, migrations 0016/0017/0018).
**Next milestone:** Invitation gate (sunsets D-035 — email-delivery prerequisite met by Session 24; broader-cohort delivery still blocked on D-036's custom-domain decision, so the gate either ships solo-operator-only first or pairs with custom-domain provisioning).
