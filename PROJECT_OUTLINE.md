...
- **Shipped sessions:** 8a–31 (including the Session 31 follow-up arc).
- **Last shipped:** Session 31 + follow-up — rail restructured around four product domains with multi-leaf groups, three coming-soon placeholder routes, breadcrumb lowercase, Research routing fixed, WorkspaceModules updated to the new taxonomy (D-047, D-048). Demo access feature scoped and documented for implementation (D-049).
- **Next milestone:** Session 32 — Knowledge reshape. Wire the rail's three Knowledge sub-leaves (Research / Vault / Sources) to real routes: `/workspace/knowledge` with Research as the default landing, `/workspace/knowledge/vault`, `/workspace/knowledge/sources`. The chat-based research surface itself comes in a later session.
- **Subsequent:** Sessions 33 (Workflows: My Workflows index + Template Library) / 34 (Integrations: Connections list + Marketplace catalog) / 35 (Help: Guides v1 + What's New changelog). Workspace dashboard deferred to Session 36+. Demo access build (~1–1.5 sessions, D-049) and the invitation gate that sunsets D-035 remain as future work, slotted when prioritized.
