...
- **Native agents:** In-app chat experiences powered by the Anthropic API today, with attached references, prompt caching, web search, per-message markdown download, and per-message Word `.docx` export (full-conversation document export deferred to the Share & connector hub, roadmap item 2). A multi-vendor-ready model abstraction layer is in place (Phase 2, via vendor-prefixed model IDs, the `lib/llm/<vendor>/` directory structure, a single-case dispatcher, and a multi-vendor pricing table); the OpenAI, Google, and other adapter implementations land in Phase 6.
...
9. **Word `.docx` export — server-side renderer, per-message export affordance, `formatted_outputs` audit row** — PARTIALLY SHIPPED. Per-message Word `.docx` export ships via the kebab destination hub on the chat message action row (Word export arc, D-054): a `renderMessageAsDocx` renderer with an agent-name title, citation footnotes, a Sources section, and a branded page footer. Full-conversation document export and the `formatted_outputs` audit row remain deferred to the Share & connector hub (roadmap item 2).
...
- **What's next:** `docs/ROADMAP.md` is the authoritative ordered list (21 prioritized items plus a 9-item backlog). Item 1 (chat attachments / knowledge upload) is the current top priority following the Word export arc closure; the operator may reorder or pivot at any time.
