...
- **Native agents:** In-app chat experiences powered by the Anthropic API today, with attached references, prompt caching, web search, per-message markdown download, per-message file attachments in the composer (chat attachments arc, D-055), and per-message Word `.docx` export (full-conversation document export deferred to the Share & connector hub, roadmap item 1). A multi-vendor-ready model abstraction layer is in place (Phase 2, via vendor-prefixed model IDs, the `lib/llm/<vendor>/` directory structure, a single-case dispatcher, and a multi-vendor pricing table); the OpenAI, Google, and other adapter implementations land in Phase 6.
...
| `connections` / `connection_grants` / `connection_policy` | shipped — workspace connector data model (per-user connections, capability grants, a per-organization policy row), RLS on each, all org-scoped (D-064, D-136) | 0044, 0066 |
...
> This table is maintained through roughly migration 0046; later schema (the BYO-model/MCP columns, the workflow tables at 0060 through 0063, the demo tables at 0064/0065, and the connection org-scoping at 0066) is not enumerated here. `supabase/migrations/` is the source of truth for the current schema.
...
7. **Per-message file upload — "+" affordance in chat input, `message-attachments` bucket and table** — SHIPPED. The composer attach affordance (a "+" trigger plus whole-surface drag-and-drop), attachment chips, an aggregate-text budget at the route layer, and an inline privacy disclosure on first attach shipped in the chat attachments arc (D-055): PDF, DOCX, TXT, MD, XLSX at up to 5 files per message and 20MB per file. Phase 2 (uploads from connected drives) is deferred to the Share & connector hub (roadmap item 1).
...
9. **Word `.docx` export — server-side renderer, per-message export affordance, `formatted_outputs` audit row** — PARTIALLY SHIPPED. Per-message Word `.docx` export ships via the kebab destination hub on the chat message action row (Word export arc, D-054): a `renderMessageAsDocx` renderer with an agent-name title, citation footnotes, a Sources section, and a branded page footer. Full-conversation document export and the `formatted_outputs` audit row remain deferred to the Share & connector hub (roadmap item 1).
...
**Superseded by the Admin arc (A1–A7, D-074 through D-084).** The Session-30 admin shell and its three early tools were rebuilt into the two-job IA (GOVERN: People, Policy & access, Audit log; MEASURE: Insights, Evals). User Access is retired (its roster, roles, deactivation, defaults, and invitation now live in People); Adoption Metrics and Productivity Calculator stay reachable-but-unlinked until the Insights MEASURE side matures. Several Phase 5 goals below are now shipped: cross-user roster and org-role management (People), an audit trail of role/status changes (Audit log), and the foundations for the rest. `lib/admin/nav.ts` remains the single source feeding the rail and landing. The remaining Phase 5 items (force-disable a user's agent, ownership transfer, agent-level audit) are forward-looking and tracked via the roadmap.
...
- **Phase status:** Phase 2 complete. The product surface is built; the polish phase (items #1 through #17), the Workspace home and rail restructure arc (six stages), the chat page redesign arc (eleven commits, 5ea1507 through 1850769), the Word export arc (three commits, cf8df0e through 5c5c811), the chat attachments arc (five commits, 7928cae through d3e42ee), the workspace home revamp and Matters arc (29 commits, 60b9e3a through f47942e), the Share & connector hub arc (Settings peer mode, Connections, the connections/grants/policy data model, Google Drive OAuth end to end, live Drive reads, the Drive picker; D-062 through D-072), and the Admin arc (A1 shell through A7 close-out; D-074 through D-084) are all closed. The Admin arc's GOVERN side is complete (Policy & access, People, Audit log); its MEASURE side is intentionally deferred (Insights A4a a functional placeholder pending a delight pass; A4b deferred pending the business-model decision; Evals A5 deferred as an open design question). Since the admin arc, the Claude for Legal content library and platform-owner tier, the full Workflows arc, the entire public marketing surface (Trust, About, Mission, Connections, FAQ, Contact, Blog, Documentation, and the Legal document drafts), demo access (D-132/D-133), and the multi-tenant connection org-scoping security fix (D-136) have all shipped and are closed. A documentation and code-health cleanup pass (ROADMAP item 6) is in progress. Active and pending work is tracked in `docs/ROADMAP.md`, which is the live source of truth.
...
- **Word export arc:** three commits extending per-message Word (.docx) export — a richer renderer (agent-name title, citation footnotes, a Sources section, a branded page footer), a kebab destination hub on the chat message action row (replacing the direct download button; the hub absorbs future export and send-to destinations as menu items), and removal of a stale "coming soon" teaser from the agent edit form. Adopted D-054 (per-message destination hub pattern). Full disposition in `CHANGELOG.md`.
- **Chat attachments arc:** five commits adding per-message file uploads to the composer — a server foundation sharing one upload-and-extract implementation with agent attachments, a composer UI (the "+" affordance, attachment chips, optimistic pending state, pre-allocated parent ids), whole-surface drag-and-drop with a depth-tracked overlay, an inline privacy disclosure on first attach, and a paperclip-to-plus affordance correction. Phase 1 supports PDF, DOCX, TXT, MD, XLSX at up to 5 files per message and 20MB per file; Phase 2 (connected drives) folds into the Share & connector hub (roadmap item 1). Adopted D-055 (client-pre-allocated parent ids). Full disposition in `CHANGELOG.md`.
- **Workspace home revamp and Matters arc:** 29 commits (60b9e3a through f47942e) reshaping the workspace home into a value-mirror of the user's work, with honest empty states. The spine, top to bottom: an editorial greeting, a two-column Today and Impact row, Matters, and Desk. The Impact band reads real `usage_events` (Agent runs, Top agent) behind a Week/Month/YTD toggle, with admin-gated Setup-needed cells for the calculator-backed metrics (D-059). The Today card (calendar) and Matters section (CLM) ship full rich connected views built and gated behind connection checks that return false today (the Reading-1 pattern, D-057, via typed connection helpers, D-058); the visible state for both is an honest Connect placeholder, and no sample data reaches users. The Recent and Tools sections were removed, their components retained unmounted (ContinueWorkingSection feeds the future Recents panel; IntegrationsRow/IntegrationCard may return as a connection-status strip). Adopted D-056 through D-061. Key files: `components/workspace/home/` (home-greeting, calendar-connect-card, today-schedule, impact-band/impact-band-client, impact-cell, timeframe-toggle, matters-section, matters-connected, reading-section, and the retained integrations-row/integration-card) and `lib/workspace/home/` (impact-math, calendar-connection, matters-connection). Full disposition in `CHANGELOG.md`.
...
- **What's next:** `docs/ROADMAP.md` is the authoritative ordered list. With the connector hub and the Admin arc both closed, the natural next major arc is the Connections phase (models-as-a-connection, the notify-and-approve lifecycle, model-retirement handling), which then gates the Business model & pricing arc, which gates the Insights cost/ROI lens (A4b). MEASURE-side delight/design passes (Insights A4a polish, Evals A5) are also available. The operator may reorder or pivot at any time.
- **Deferred strategic decision:** per D-051, the out-of-scope C4L plugins (law-student, legal-clinic, legal-builder-hub, cocounsel-legal) stay deferred unless a trigger fires; they sit at roadmap item 4.
